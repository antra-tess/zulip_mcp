...
test('a message from a stream created after startup carries its descriptor, once', async () => {
  // Discovery saw only #general; #info-exchange is created later and its
  // first message (a mention of the bot) arrives live.
  const events = [
    { id: 1, type: 'message', flags: [], message: raw(10) },
    { id: 2, type: 'message', flags: ['mentioned'], message: raw(11, { display_recipient: 'info-exchange', stream_id: 9, content: '@**Bot** hi' }) },
    { id: 3, type: 'message', flags: [], message: raw(12, { display_recipient: 'info-exchange', stream_id: 9 }) },
  ...
  assert.deepEqual(seen, [
    ...
    { id: '11', channelId: 'zulip:info-exchange', newChannel: 'zulip:info-exchange', address: { stream_name: 'info-exchange', stream_id: 9 } },
    // Already described: not announced again.
    { id: '12', channelId: 'zulip:info-exchange', newChannel: undefined, address: undefined },
  ]);
});
